			...
			const uint16_t manuf1[2], const uint16_t manuf2[2])
{
	...
	strncpy((char *) atval, desc_out_temp, len+1);
	attrib_db_add(adapter->adapter, h++, &uuid, ATT_NONE, ATT_NOT_PERMITTED,
								atval, len);
	...
	strncpy((char *) atval, desc_out_hum, len+1);
	attrib_db_add(adapter->adapter, h, &uuid, ATT_NONE, ATT_NOT_PERMITTED,
								atval, len);
							...
							uint16_t range[2])
{
	...
	strncpy((char *) atval, manufacturer_name1, len+1);
	attrib_db_add(adapter->adapter, h++, &uuid, ATT_NONE, ATT_NOT_PERMITTED,
								atval, len);
	...
	len = strlen(serial1);
	strncpy((char *) atval, serial1, len+1);
	attrib_db_add(adapter->adapter, h, &uuid, ATT_NONE, ATT_NOT_PERMITTED,
								atval, len);
							...
							uint16_t range[2])
{
	...
	strncpy((char *) atval, manufacturer_name2, len+1);
	attrib_db_add(adapter->adapter, h++, &uuid, ATT_NONE, ATT_NOT_PERMITTED,
								atval, len);
	...
	bt_uuid16_create(&uuid, MANUFACTURER_SERIAL_UUID);
	len = strlen(serial2);
	strncpy((char *) atval, serial2, len+1);
	attrib_db_add(adapter->adapter, h, &uuid, ATT_NONE, ATT_NOT_PERMITTED,
								atval, len);
						...
						const uint16_t vendor[2])
{
	...
	strncpy((char *) atval, desc_weight, len+1);
	attrib_db_add(adapter->adapter, h, &uuid, ATT_NONE, ATT_NOT_PERMITTED,
								atval, len);
	g_assert(h - start_handle + 1 == svc_size);
