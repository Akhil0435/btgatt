...
int main(int argc, char *argv[])
{
	...
	ptr = strchr(ptr, '/');
	if (ptr) {
		size_t service_length = strlen(ptr + 1);
                if (service_length  >= sizeof(service) ) {
                     fprintf(stderr, "ERROR: Unable to allocate memory\n");
                       return CUPS_BACKEND_FAILED;
                }
                strncpy(service, ptr + 1, sizeof(service) - 1);
		/*(strncpy(service, ptr + 1, 256);*/
		...
		if (!strncasecmp(ptr + 1, "spp", 3))
			...
			proto = 0;
                service[sizeof(service) - 1] = '\0'; // Null-terminate the string
	} else {
		strcpy(service, "auto");
		proto = 0;
	}
