...
static void update_adapter_wrapper(gpointer data, gpointer user_data) {
    update_adapter((struct hdp_adapter *)data);
}
static void remove_application(struct hdp_application *app)
{
	...
	g_slist_foreach(adapters, (GFunc) update_adapter_wrapper, NULL);
}
...
static void hdp_application_unref_wrapper(gpointer data, gpointer user_data) {
    hdp_application_unref((struct hdp_application *)data);
}
static void manager_path_unregister(gpointer data)
{
	g_slist_foreach(applications, (GFunc) hdp_application_unref_wrapper, NULL);
