...
static void shutdown_mdl_wrapper(gpointer data, gpointer user_data) {
    shutdown_mdl((struct mcap_mdl *)data);
}

static void mcap_mdl_unref_wrapper(gpointer data, gpointer user_data) {
    mcap_mdl_unref((struct mcap_mdl *)data);
}
static void close_mcl(struct mcap_mcl *mcl, gboolean cache_requested)
{
	...
	g_slist_foreach(mcl->mdls, (GFunc) shutdown_mdl_wrapper, NULL);
	...
	g_slist_foreach(mcl->mdls, (GFunc) mcap_mdl_unref_wrapper, NULL);
