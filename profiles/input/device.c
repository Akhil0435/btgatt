...
static int create_hid_dev_name(sdp_record_t *rec, struct hidp_connadd_req *req)
{
	...
	if (sdp_get_service_desc(rec, sdesc, sizeof(sdesc)) == 0) {
		...
		if (sdp_get_provider_name(rec, pname, sizeof(pname)) == 0 &&
			...
			printf("1");
			//snprintf(req->name, sizeof(req->name), "%s %s", pname, sdesc);
		else
...
static int uhid_connadd(struct input_device *idev, struct hidp_connadd_req *req)
{
	...
	/*strncpy((char *) ev.u.create.name, req->name,
	 ...
	 // Ensure the length argument to strncpy does not exceed the size of ev.u.create.name
        name_len = strlen(req->name);
	copy_len = name_len < sizeof(ev.u.create.name) ? name_len : sizeof(ev.u.create.name) - 1;
       printf("name_len: %zu, copy_len: %zu, sizeof(ev.u.create.name): %zu\n", name_len, copy_len, sizeof(ev.u.create.name));

        strncpy((char *)ev.u.create.name, req->name, copy_len);
        ev.u.create.name[copy_len] = '\0'; // Ensure null-termination
